...
const intervalInSeconds = parseInt(process.env.DRONE_INTERVAL) || 8; 
...
async function handler() {
  try {
    ...
    const repos = await doRequest('/api/user/repos?latest=true') as Array<Repo>;
    const activeRepos = repos.filter(repo => repo.active);

    for (const repoIndex in activeRepos) {
      const repo = repos[repoIndex];

      const builds = await doRequest(`/api/repos/${repo.slug}/builds?page=1`) as Array<Build>;
